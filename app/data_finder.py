...
class CGUsDataFinder: 
    ...
    def __init__(self, service: str, doc_type: str):
        ...
        self.versions = {
            datetime.strptime(file_path.name.rstrip(".md"), DATASET_DATE_FORMAT): file_path
    ...
    def __validate_path(path: Path):
        if not Path(CGUS_DATASET_PATH).resolve() in path.resolve().parents or not path.exists():
